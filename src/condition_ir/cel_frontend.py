...
that canonical form into the typed ``ConditionIR`` (which uses bare
...
Call, BytesLit, NullLit) raise ``TypeError`` with the source span. The IR
can grow into these as needed; today they are out of scope.
"""
...
from condition_ir.cel_types import CelExpr, to_cel_expr
from condition_ir.registry import (
    ConceptInfo,
    KindType,
    condition_registry_fingerprint,
)
from condition_ir.checked import CheckedCondition
